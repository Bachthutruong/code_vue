      ...
      <div class="modal-invite-body">
        <div class="row q-col-gutter-md">
          ...
          >
            ...
            <div v-else>
              <span class="text-channel">{{ $t('workspace.channel.to') }}</span>
              <q-chip
                size="20px"
                square
                class="chip-channel"
              >
                ...
                <span class="text-body-regular text-neutral">
                  {{ channelDetail?.name || '' }}
                </span>
              </q-chip>
            </div>
        ...
        >
          <div class="box-detail-mark-done">
            ...
            <q-separator />
            <div class="box-detail-mark-done-action">
              <div>
                <q-checkbox
...
import { IMember } from 'src/model/Global';
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const infoChannel = computed(() => {
      return channelStore.detailChanel;
    });
    const channelDetail = computed(() => {
      const member = infoChannel?.value?.members?.[0] as IMember;
      return {
        ...infoChannel,
    ...
    const setInfoModal = async (itemId: any, type?: string) => {
      ...
      await TaskService.getDetailTask(`${itemId}`).then((res: any) => {
        if (res?.status_code === 200) {
          ...
          const read = info?.reads?.[0]?.is_read || false;
