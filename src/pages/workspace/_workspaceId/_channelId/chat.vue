  ...
  setup() {
    ...
    const refactorItem = (item: any) => {
      if (item?.is_activity) {
        ...
        if (['JOIN', 'LEAVE'].includes(item?.type)) {
          return dataTask;
        }
        dataTask = {
          ...dataTask,
    ...
    const getMenu = (item: formChat) => {
      ...
      const menu = [
        ...
        },
        {
          ...
          show:
            (permissionEditAndRemoveChat.value || item.user?.id === infoUser.value?.id) &&
            !item.deleteChat?.isDeleted,
      ...
      getAllMessage().then(() => {
        ...
        if (route.query?.chatId) {
          ...
          if (route.query?.pollId) {
            showPoll(Number(route.query?.pollId));
          }
        }
        ...
        >
          ...
          >
            ...
            <add-or-remove-member
              v-if="item?.isActivity && ['JOIN', 'LEAVE'].includes(<string>item?.type)"
              :info-task="item"
            />
            <title-pin
